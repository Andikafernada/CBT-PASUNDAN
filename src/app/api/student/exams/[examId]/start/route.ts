...
export async function POST(
  ...
  try {
    ...
    if (!isSuperReviewer && examCategory === "PKL" && !isStudentPkl) {
    ...
    if (!isSuperReviewer && examCategory === "REGULER" && isStudentPkl) {
    ...
    if (!isSuperReviewer && exam.examGroups && exam.examGroups.length > 0) {
    ...
    if (!isSuperReviewer && studentGroupRel && (studentGroupRel.startTime || studentGroupRel.endTime)) {
      ...
      if (!isSuperReviewer && exam.startTime && currentTime < new Date(exam.startTime)) {
      ...
      if (!isSuperReviewer && exam.endTime && currentTime > new Date(exam.endTime)) {
    ...
    if (!isSuperReviewer && exam.requireKioskBrowser && !isBypassed) {
    ...
    // 📋 Enforce Learning Survey Completion before starting a new exam session
    if (!session && !isSuperReviewer) {
      const surveyCompleted = await prisma.learningSurvey.findUnique({
        where: {
          examId_userId: {
            examId: exam.id,
            userId: user.id,
      ...
      if (!surveyCompleted) {
        return NextResponse.json(
          {
            error: "Anda wajib mengisi Kuesioner Pembelajaran terlebih dahulu sebelum memulai ujian.",
            requiresSurvey: true,
          },
          { status: 403 }
        );
    ...
    return NextResponse.json({
      ...
      session: {
        ...
        studentGroup: user.group?.name || (isSuperReviewer ? "Super Siswa" : "Siswa"),
