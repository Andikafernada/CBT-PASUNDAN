    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-xl sm:text-2xl font-extrabold text-slate-900 dark:text-black tracking-tight">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <button
            ...
            className="px-3.5 py-2 rounded-xl bg-white dark:bg-sky-50 hover:bg-slate-50 dark:hover:bg-sky-100 border border-slate-200 dark:border-sky-200 text-slate-700 dark:text-slate-200 text-xs font-bold flex items-center gap-2 shadow-xs transition"
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="glass p-5">
          ...
          <div className="text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-black">
      ...
      {/* Filter & Search Bar */}
      <div className="glass p-4 flex flex-col md:flex-row items-center justify-between gap-3">
        <form onSubmit={handleSearchSubmit} className="relative w-full md:w-80">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 bg-slate-50 dark:bg-sky-50 border border-slate-200 dark:border-sky-200 rounded-xl text-xs text-slate-900 dark:text-black placeholder-slate-400 focus:outline-none focus:border-blue-500"
          />
        </form>
        ...
        <div className="flex items-center gap-2.5 w-full md:w-auto flex-wrap">
          ...
          <div className="flex items-center gap-1.5 text-xs text-slate-600 dark:text-slate-400">
            ...
            <select
              ...
              className="px-3 py-1.5 bg-slate-50 dark:bg-sky-50 border border-slate-200 dark:border-sky-200 rounded-xl text-xs text-slate-800 dark:text-slate-200 font-medium focus:outline-none focus:border-blue-500"
            >
      ...
      <div className="glass overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead className="bg-slate-50 dark:bg-sky-50 text-slate-600 dark:text-slate-400 border-b border-slate-200 dark:border-sky-200">
              <tr>
              ...
              {loading ? (
                ...
                data.students.map((student: any) => {
                  ...
                  return (
                    <tr
                      key={student.id}
                      className="hover:bg-slate-50/80 dark:hover:bg-sky-100 transition cursor-pointer"
                      onClick={() => setSelectedStudent(student)}
                    >
                      <td className="py-3.5 px-4">
                        <div className="font-bold text-slate-900 dark:text-black text-sm">{student.name}</div>
                        <div className="text-[11px] text-slate-500 dark:text-slate-400 font-mono">NIS: {student.nis}</div>
                      </td>

                      <td className="py-3.5 px-4">
                        <span className="px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-sky-50 text-slate-700 dark:text-slate-300 font-semibold border border-slate-200 dark:border-sky-200">
                          {student.groupName}
                        </span>
                      </td>
                      ...
                      <td className="py-3.5 px-4 text-right">
                        <button
                          ...
                          className="px-3 py-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-sky-50 dark:hover:bg-slate-700 text-blue-600 dark:text-blue-400 font-bold text-xs inline-flex items-center gap-1.5 transition"
                        >
                          <span>Rincian</span>
      ...
      {selectedStudent && (
        <div className="fixed inset-0 z-50 bg-sky-950/25 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="glass max-w-2xl w-full p-6 sm:p-7 relative max-h-[90vh] overflow-y-auto">
            <button
            ...
            <div className="flex items-start gap-3.5 mb-6 pb-4 border-b border-slate-100 dark:border-sky-200">
              ...
              <div>
                <h3 className="font-extrabold text-lg text-slate-900 dark:text-black">{selectedStudent.name}</h3>
            ...
            <div className="space-y-4">
              ...
              {selectedStudent.subjectBreakdowns?.length === 0 ? (
                <div className="p-6 text-center rounded-2xl bg-slate-50 dark:bg-sky-50 border border-slate-200 dark:border-sky-200 text-slate-500 text-xs">
                ...
                selectedStudent.subjectBreakdowns?.map((sub: any, idx: number) => {
                  return (
                    <div
                      key={idx}
                      className="p-4 rounded-2xl bg-slate-50 dark:bg-sky-50 border border-slate-200/80 dark:border-sky-200 space-y-3"
                    >
                      {/* Subject Header */}
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="font-extrabold text-sm text-slate-900 dark:text-black">{sub.subjectName}</span>
                      ...
                      <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-slate-200/60 dark:border-sky-200 text-xs">
            ...
            <div className="mt-6 pt-4 border-t border-slate-100 dark:border-sky-200 flex justify-end">
              <button
                onClick={() => setSelectedStudent(null)}
                className="px-5 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-sky-50 dark:hover:bg-slate-700 text-slate-700 dark:text-black font-bold text-xs transition"
              >
                Tutup
              </button>
            </div>
