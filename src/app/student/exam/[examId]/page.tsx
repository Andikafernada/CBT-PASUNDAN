...
export default function ExamRoomPage({
  ...
  // 🛡️ Sync server timer with monotonic guard against timer reset / inflation

  const syncServerTimer = (newServerRemaining: number, isProctorExtended = false) => {

    if (typeof newServerRemaining !== "number" || isNaN(newServerRemaining)) return;



    // Current local countdown in seconds

    const currentSec = Math.max(0, Math.round((targetEndTimeRef.current - Date.now()) / 1000));



    // 🛡️ Monotonic Guard:

    // If newServerRemaining is greater than current local time + 15s (network grace),

    // reject it unless confirmed as an intentional proctor time extension!

    if (currentSec > 0 && newServerRemaining > currentSec + 15 && !isProctorExtended) {

      console.warn(`[Timer Guard] Rejected server timer inflation from ${currentSec}s to ${newServerRemaining}s`);
    ...
    targetEndTimeRef.current = Date.now() + newServerRemaining * 1000;

    try {

      localStorage.setItem(`cbt_target_end_${examId}`, String(targetEndTimeRef.current));

    } catch {}

    setRemainingSeconds(newServerRemaining);
    ...
    const statusInterval = setInterval(async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          // Real-time server timer synchronization

          const serverTime = data.serverRemainingSeconds ?? data.remainingSeconds;

          if (typeof serverTime === "number") {

            const localTime = Math.max(0, Math.round((targetEndTimeRef.current - Date.now()) / 1000));
  ...
  // 🔄 SAFE REFRESH / RELOAD HANDLER (Aman untuk Exambro di Lab: Bebas Pelanggaran & Sync Jawaban)
  ...
  const handleSafeRefresh = async () => {
    ...
    try {
      ...
      // Flush any pending unsaved answers first so nothing is lost

      await flushPendingQueue();

      if (questions && questions.length > 0) {

        localStorage.setItem(`cbt_backup_${examId}`, JSON.stringify(questions));

      }

    } catch (e) {

      console.warn("Safe refresh flush error:", e);
  ...
  useEffect(() => {
    ...
    const handleOnline = async () => {
      ...
      console.log("[Network] Reconnected to online. Syncing queue & authoritative timer...");

      await flushPendingQueue();



      // Immediately fetch authoritative server time to prevent any client-side drift or stale timer
      ...
      try {
        ...
        if (res.ok) {
          ...
          const serverTime = st.serverRemainingSeconds ?? st.remainingSeconds;
    ...
    try {
      ...
      if (bulkResult.success && bulkResult.syncedCount > 0) {
        ...
        if (typeof bulkResult.serverRemainingSeconds === "number") {
          ...
          syncServerTimer(bulkResult.serverRemainingSeconds);
      ...
      for (const item of pending) {
        ...
        if (res.ok) {
          ...
          if (resData && typeof resData.serverRemainingSeconds === "number") {
            ...
            syncServerTimer(resData.serverRemainingSeconds);
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (typeof data.serverRemainingSeconds === "number") {
            ...
            syncServerTimer(data.serverRemainingSeconds);
  ...
  // Helper with debouncing to prevent flood of violations (Immunity for PKL/Bebas Pelanggaran/Exambro)
  ...
  const reportViolationDebounced = (type: string, detail: string) => {
    ...
    const isExambroClient = /Exambro|SafeExamBrowser|CBT-Exambro|KioskLab|SEB|ExamBrowser|Candy/i.test(ua);

    if (isExambroClient && (type === "WINDOW_BLUR" || type === "TAB_SWITCH" || type === "FULLSCREEN_EXIT")) {

      console.log(`[Exambro Shield] Suppressing ${type} violation inside locked Exambrowser kiosk environment.`);
  ...
  // 🔒 HARDENED ANTI-CHEAT WITH SMART MOBILE & EXAMBROWSER IMMUNITY
  ...
  useEffect(() => {
    ...
    // 🛡️ Deteksi apakah siswa menggunakan Exambrowser (Kiosk Lab)

    const isExambrowser = typeof window !== "undefined" && (

      /Exambro|SafeExamBrowser|CBT-Exambro|KioskLab|SEB|ExamBrowser|Candy/i.test(ua)
    ...
    const handleVisibilityChange = () => {
      ...
      // 🛡️ Di Exambrowser Lab, siswa terkunci di dalam aplikasi Kiosk tanpa tab & tanpa akses taskbar.

      // Seringkali event render internal Exambrowser memicu hidden palsu; jangan hukum siswa Exambro!
      ...
      // Hanya rekam pelanggaran jika dokumen benar-benar disembunyikan (Beralih Tab / Minimize)

      if (document.hidden) {

        // Toleransi Layar Mati / Idle Screen Lock (> 45 detik tidak aktif)
        ...
        fetch(`/api/student/exams/${examId}/status`)
          ...
          .then((st) => {
            ...
            if (st?.status === "FORCE_FINISHED" || st?.isForceFinished) {

              handleForceFinishedByAdmin({

                examTitle: st.examTitle || exam?.title,

                subjectName: st.subjectName || exam?.subject,
    ...
    const handleBlur = (e: FocusEvent) => {
      ...
      // 🛡️ Exambro & Mobile Shield:

      // Di Exambrowser maupun HP, event blur sering terpanggil saat klik opsi, klik palet, render CEF/WebView2.

      // Karena Exambro sudah mengunci OS di level Windows, event blur ini 100% aman diabaikan.

      if (isMobileDevice || isExambrowser) {
      ...
      // Untuk browser standar non-exambro:

      // Jangan pernah menghukum blur jika dokumen masih terlihat di layar (!document.hidden).

      // Perpindahan tab sesungguhnya 100% dideteksi secara akurat oleh visibilitychange di atas.
    ...
    const handleFullscreenChange = () => {
      ...
      // 📱 HP (PKL) dan 🛡️ Exambro Lab: Keduanya bebas dari sanksi keluar fullscreen HTML5:

      // - HP: Sering terganggu keyboard / status bar

      // - Exambrowser: Sudah berstatus Fullscreen / Borderless Kiosk di level desktop Windows
      ...
      // Hanya berlakukan sanksi keluar fullscreen pada Browser Biasa (Chrome/Edge biasa) di PC Lab
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // 2. Safe Reload / Refresh on F5 or Ctrl+R (Aman: sync jawaban & reload tanpa sanksi pelanggaran)
  ...
  const initExam = async () => {
    ...
    try {
      ...
      let initialTime = typeof data.exam?.remainingSeconds === "number" ? data.exam.remainingSeconds : 3600;



      // 🛡️ ANTI-TIMER RESET & OFFLINE TARGET END RECOVERY:

      // If we have a stored target end time in localStorage, calculate remaining seconds from it!

      const storedEndTimeStr = typeof window !== "undefined" ? localStorage.getItem(`cbt_target_end_${examId}`) : null;

      const storedEndTime = storedEndTimeStr ? Number(storedEndTimeStr) : 0;



      if (storedEndTime && storedEndTime > Date.now()) {

        const calculatedFromStored = Math.max(0, Math.round((storedEndTime - Date.now()) / 1000));

        // If recovered offline from cache or if stored time is less than initialTime:

        if (!navigator.onLine || calculatedFromStored < initialTime) {

          initialTime = calculatedFromStored;
  ...
  return (
    ...
    <div translate="no" className="notranslate exam-workspace min-h-screen bg-slate-50 text-slate-900 flex flex-col select-none transition-colors duration-150">
      ...
      <header className="border-b border-slate-200 bg-white/95 backdrop-blur-md sticky top-0 z-30 shadow-xs">
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-6 py-2 sm:py-2.5 flex items-center justify-between gap-2">
          {/* Left: Palette Button + Compact Student Box */}
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            <button
              onClick={() => setShowPalette(!showPalette)}
              className="lg:hidden p-2 rounded-xl bg-sky-50 text-sky-700 border border-sky-200 hover:bg-sky-100 flex-shrink-0"
              title="Daftar Soal"
            >
            ...
            {/* Identitas Siswa */}
            <div className="flex items-center gap-2 px-2.5 py-1 sm:px-3 sm:py-1.5 bg-sky-50 border border-sky-200 rounded-xl flex-shrink min-w-0">
              <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-sky-500 text-white flex items-center justify-center font-black text-xs flex-shrink-0">
                <User className="w-4 h-4 text-white" />
              </div>
              <div className="leading-tight min-w-0">
                <div className="flex items-center gap-1.5">
                  <span className="font-black text-xs sm:text-sm text-slate-900 truncate max-w-[110px] sm:max-w-[180px]">
                    {cleanStudentName}
                  </span>
                  <span className="text-[9px] sm:text-[10px] uppercase font-black px-1.5 py-0.2 rounded bg-sky-500 text-white flex-shrink-0">
                    {studentGroup}
                  </span>
                </div>
                <div className="text-[10px] text-sky-900 font-bold hidden sm:block mt-0.5">
                  NIS: {studentNis}
                </div>
              </div>
            </div>

            {/* Desktop Center: Title & Progress (Shown on lg+) */}
            <div className="hidden lg:block border-l border-slate-200 pl-3 min-w-0">
              <div className="flex items-center gap-2">
                <h1 className="font-black text-sm lg:text-base text-slate-900 tracking-tight truncate max-w-[280px] xl:max-w-[420px]">
                  {exam?.title}
                </h1>
                {exam?.category === "PKL" && (
                  <span className="px-1.5 py-0.5 text-[10px] font-bold bg-purple-100 text-purple-950 border border-purple-300 rounded-md flex-shrink-0">
                    PKL
                  </span>
                )}
                {exam?.disableAntiCheat && (
                  <span className="px-1.5 py-0.5 text-[10px] font-bold bg-amber-100 text-amber-950 border border-amber-300 rounded-md flex-shrink-0" title="Bebas Pelanggaran Aktif">
                    🛡️ Bebas Pelanggaran
                  </span>
                )}
              </div>
              <div className="flex items-center gap-2 text-xs text-slate-500">
                <span className="font-bold text-slate-800">{exam?.subject || "Ujian"}</span>
                <span>•</span>
                <span className="font-bold text-sky-700">Soal {currentIndex + 1} dari {questions.length}</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2.5 flex-shrink-0">
            ...
            <div className="hidden md:flex items-center bg-slate-100 border border-slate-200 rounded-xl p-1 text-xs">
              <button onClick={() => setFontSize("sm")} className={`px-2 py-1 rounded-lg transition ${fontSize === "sm" ? "bg-blue-600 text-white font-bold" : "text-slate-500"}`}>A-</button>
              <button onClick={() => setFontSize("base")} className={`px-2 py-1 rounded-lg transition ${fontSize === "base" ? "bg-blue-600 text-white font-bold" : "text-slate-500"}`}>A</button>
              <button onClick={() => setFontSize("lg")} className={`px-2 py-1 rounded-lg transition ${fontSize === "lg" ? "bg-blue-600 text-white font-bold" : "text-slate-500"}`}>A+</button>
            </div>
            ...
            <button
              ...
              className="flex items-center gap-1 px-2 sm:px-2.5 py-1.5 rounded-xl bg-slate-100 hover:bg-sky-100 text-slate-700 hover:text-blue-700 border border-slate-200 text-xs font-bold transition shadow-2xs cursor-pointer active:scale-95"
              title="Segarkan Soal"
            >
            ...
            {/* Autosave Status (Tablet/Desktop) */}
            <div className="hidden sm:flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-slate-100 text-[11px] text-slate-600 border border-slate-200">
              {saveStatus === "SAVING" ? (
                <>
                ...
                <>
                  ...
                  <span className="text-emerald-700 font-semibold hidden md:inline">Tersimpan</span>
                </>
              )}
            </div>

            {/* Countdown Timer (ALWAYS PROMINENT & VISIBLE ON MOBILE & DESKTOP!) */}
            <div
              className={`flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3.5 py-1.5 rounded-xl font-mono text-xs sm:text-sm font-black border shadow-xs transition ${
                isTimerCritical
        ...
        {/* Tier 2: Mobile Sub-bar (lg:hidden) - Clean & Dedicated for Exam Title & Question Progress */}
        <div className="lg:hidden border-t border-slate-200/80 bg-slate-50/95 px-3 py-1.5 flex items-center justify-between gap-2 text-xs">
